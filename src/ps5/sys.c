...
spawn_embedded(const char *label, const uint8_t* elf, size_t elf_size) {
  ...
  if(pid < 0) {
    ...
    notify("sonic-loader: failed to spawn %s", label);
    fprintf(stderr, "sonic-loader: failed to spawn %s\n", label);
    return -1;
  }
...
spawn_embedded_argv(const uint8_t *elf, size_t elf_size, char **argv) {
  ...
  if(pid < 0) {
    notify("sonic-loader: failed to spawn %s", argv[0] ? argv[0] : "(elf)");
    fprintf(stderr, "sonic-loader: failed to spawn %s\n",
            argv[0] ? argv[0] : "(elf)");
    return -1;
  }
...
sys_spawn_embedded_payloads(void) {
  ...
  if(stat(KSTUFF_INSTALL_PATH, &st) != 0 || st.st_size <= 4) {
    ...
    if(fw == 0) {
      notify("Sonic Loader: couldn't read firmware version — falling "
             "back to EchoStretch combo.");
    } else {
      notify("Sonic Loader first boot: detected firmware %u.%02u — "
    ...
    if(kr == 0 && sr == 0) {
      notify("Sonic Loader: auto-install OK (kstuff %s + SMP %s). "
             "Loading kstuff now.",
             use_drakmor ? "drakmor" : "EchoStretch", smp_v);
    } else {
      notify("Sonic Loader: auto-install failed (kstuff=%d, SMP=%d). "
  ...
  if(stat(KSTUFF_INSTALL_PATH, &st) == 0 && st.st_size > 4) {
    ...
    notify("Sonic Loader: no kstuff installed yet. Open Homebrew Loader "
  ...
  if(smp_buf) {
    ...
    notify("Sonic Loader: ShadowMountPlus not installed yet. Open "
  ...
  if(first_boot) {
    ...
    /* Fire the "go install kstuff" toast a second time after a small
       ...
       the install prompt. */
    sleep(3);
    notify("Sonic Loader first boot: open Settings → Install kstuff-lite "
